[x] upgrade bbsengine.org to use bootstrap.php (to fix Smarty.class.php include error) - DONE
   ...
   - Files updated in www/org/php/: index.php, login.php, logout.php, register.php, post.php, page.php, archive.php, download.php, dir.php, about.php, testform.php, phpinfo.php, bbsenginedotorg.php, gencaptchaimage.php
...
[x] Fix /handbook/6/ 500 internal server error - handbook.php retired; /handbook/<v>/... routes through engine/router.php with TEOSURL/TEOSDIR set from URI-prefix detection in the HTTP entry-point (@since 20250623, resolved 2026-09-07).
...
[x] `deploy-tui` hard-fails on editable-in-venv; port `verify-install` (`@since 20260828`)
   ...
   - Fix (`py/src/Makefile precheck-editable`): inspect `pip show` for `Editable project location:` and bail non-zero with the two clean remedies (`deploy --editable bbsengine6.tui` to stay in editable mode, `pip uninstall bbsengine6 && deploy bbsengine6.tui` to switch to wheels, `deploy --with-deps bbsengine6.tui` to override for one run). Implemented as a recipe-time macro so it only fires on the non-editable branch.
   - `--with-deps` escape hatch (`py/src/Makefile precheck-editable`, `DEPLOY_WITH_DEPS=1` branch): the `--with-deps` flag semantically promises "rebuild the venv, no questions asked", so blocking on a mixed editable/wheel state under that flag is wrong-headed. With `DEPLOY_WITH_DEPS=1` set, `precheck-editable` warns and proceeds; the post-install `verify-install` macro then becomes the actual correctness check. Verified end-to-end: warning fires, `pip install` runs, `verify-install` catches the silent no-op and exits non-zero with `expected ... 0.0.1.dev<ts>, pip show Version: 0.0.1.dev<editable-ts>` — pointing the operator at the editable-install cause without the Makefile ever calling `pip uninstall` itself.
   - `DEPLOY_WITH_DEPS` plumbing (`deploytool/src/deploytool/lib.py` `run_make_deploy`): set when `--with-deps` is passed, stripped from inherited env when not (mirrors the `DEPLOY_EDITABLE` single-source-of-truth contract). Three new `tests/test_deploy_with_deps.py` cases pin the contract.
   ...
   - Cross-ref: `bbsengine6/CHANGELOG.md` "[Unreleased] build: deploy-tui hard-fails when the active venv has an editable install"; `zoidoffice/src/Makefile VERIFY_INSTALL`; `casino/Makefile:verify-install`; `deploytool/SPECS.md` §2.2.1 (`DEPLOY_WITH_DEPS` env-var contract).
...
[x] member auth hot path: use psycopg3 `%s` + `cur.execute(sql, params)` instead of `database.query("$1", …) (`@since 20260821`)
   ...
   - Cross-ref: `zoid6/TODO.md` "Password column hardening — legacy MD5-crypt migration" — the auth hot path migration above fixes one failure mode (the `$1` → `sql.Literal` substitution cascade). The runtime audit (`audit_password_hash` in `member.checkpassword`), the legacy-MD5 detection / CHECK constraint, and the `.pth` cleanup have all landed (2026-08-22; see the `[x]` entry below). The work split between the two repos held: this repo owns the schema, the SQL layer, and the runtime diagnostic; zoid6 owns the operator-facing audit-and-migrate workflow plus the venv `.pth` cleanup (a venv-hygiene issue, not a schema one). Cross-ref also to `casino/TODO.md` "test fixture migration: gen_salt('md5') → gen_salt('bf')" — the new `chk_member_password_bcrypt` constraint will reject the `$1$` hashes that 9 casino test files currently seed via `crypt('test', gen_salt('md5'))` once the constraint is applied.

[x] See `zoid6/TODO.md` "Password column hardening — legacy MD5-crypt migration (@since 20260822)" — runtime audit + CHECK constraint + .pth cleanup. Owned by zoid6; this repo is the upstream schema authority. Back-reference from the auth hot path migration above.
   - **Closed 2026-08-22** in this repo: `bbsengine6.member.audit_password_hash` (per-auth diagnostic), `bbsengine6.member.audit_password_column` (one-shot migration scan), `chk_member_password_bcrypt` CHECK constraint in `bbsengine6/sql/manage_password_format.sql` (new; `\i`-included from `bbsengine6.sql` after `member.sql`), `bbsengine6/sql/member.sql` top-of-file comment documenting the bcrypt-only invariant, and `bbsengine6/py/tests/test_member_audit_password_hash.py` + `bbsengine6/py/tests/test_member_legacy_hash_audit.py` (22 unit tests + 1 live-DB pin). `member.checkpassword` now invokes `audit_password_hash` immediately before the bcrypt round-trip SELECT on the same cursor, so every auth attempt (bed `auth login`/`auth reconnect`, casino auth prompt, future entry points) emits one diagnostic line. See `bbsengine6/CHANGELOG.md` "[Unreleased] member + sql: password column hardening" for the full diff. The .pth cleanup item lives in `zoid6/TODO.md` "Clean up stale editable-install `.pth` files" since it's a venv-hygiene issue, not a schema one.
...
- [x] Make `python -m bbsengine6.startup` self-heal when the target database does not yet exist. 2026-08-20.
  - Resolution: `py/src/bbsengine6/startup/main.py` now runs a **pre-flight** before any stage. It builds an admin pool against `dbname="postgres"` (constant `_ADMIN_DATABASE` at module top), then dispatches `stage_zero` via `lib.runmodule(args, "stage_zero")` exactly as before. After stage_zero returns success the function picks a target pool for `stage_one`:
    - If the caller supplied `pool=` via kwargs and `pool.getconn().info.dbname == args.databasename`, the caller's pool is reused (and the conn is returned to the pool).
    - If the caller supplied `pool=` whose dbname does not match (e.g. an old `postgres` admin pool) or whose `getconn()` raised `OperationalError` / `OSError` / `psycopg_pool.PoolTimeout`, the pool is discarded and a fresh `database.getpool(args, dbname=args.databasename)` is used.
    - Otherwise a fresh target pool is built against `args.databasename`.
  - A caller-supplied `conn=` is no longer reused for stage_one — the pre-flight flow owns the conn lifecycle. The `conn` kwarg is still accepted for backward compatibility but is dropped before stage dispatch.
  - Caveats addressed:
    - `CREATE DATABASE` autocommit remains the responsibility of `backend/checkdatabase.py:53` — unchanged.
    - Reserved-name guard (`postgres`, `template0`, `template1`) is unchanged — `database.exists` returns True for them, so `checkdatabase` skips the create.
    - Routing is explicit: stage_zero always sees a `postgres` admin pool (built internally by `stage_zero.py` and now also built by the new pre-flight for diagnostic purposes). Stage_one gets the target-DB pool by the rules above.
  - Tests: `py/tests/test_startup_zoid6_missing.py` (the existing file, extended). All 15 mock tests pass; integration-class tests require a live DB. New tests pin pre-flight semantics: caller-pool reuse vs. replacement, admin-pool failure, target-pool failure, and no-stderr-noise (no `psycopg_pool` "error connecting" warnings during pre-flight).
  - Cross-ref: `zoid6/TODO.md` and `zoid6/SPEC.md §9` — DB lifecycle is intentionally owned by `bbsengine6.startup`, not `zoid6`.
