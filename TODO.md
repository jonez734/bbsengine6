...
- [ ] Make `python -m bbsengine6.startup` self-heal when the target database does not yet exist.
  - Today: `py/src/bbsengine6/startup/main.py:118-144` builds the first pool against `args.databasename` *before* any stage runs. A missing DB raises `psycopg.OperationalError("database ... does not exist")`, which is caught at line 128-139, logged as "pool is None", and `main` returns False — so the user never reaches `stage_zero`, even though `stage_zero` already knows how to create the missing DB via the `postgres` maintenance-DB pool + `checkcreatedb` + `checkdatabase`.
  - Desired: reorder so the maintenance-DB path runs first. Two equivalent ways (pick one during implementation):
    1. In the `if conn is None / pool is None` branch (lines 118-144), build the initial pool against `dbname="postgres"` (mirroring `backend/stage_zero.py:25`), then call `lib.runmodule(args, "stage_zero", pool=postgres_pool, ...)` — `checkcreatedb` + `checkdatabase` will create the missing DB as needed. After stage_zero succeeds, build a fresh pool against the now-present `args.databasename` and run `stage_one` / `engine` / `bank` against it.
    2. Or: move the conn/pool acquisition inside `_work()` so each stage gets the pool it needs (stage_zero always gets a `postgres` pool; later stages get `args.databasename`). Then delete the early pool-build entirely.
  - Caveats:
    - `CREATE DATABASE` cannot run in a transaction; `backend/checkdatabase.py:53` already sets `conn.autocommit = True` — do not regress that.
    - If the missing DB is one of the reserved Postgres DBs (`postgres`, `template0`, `template1`), refuse to create it — `checkdatabase.main` already rejects these via `database.exists` returning True for `postgres`; verify the same guard fires for `template0`/`template1`.
    - The `pool`/`conn` plumbing in `startup/main.py:72-82` and the `BACKEND_STAGES` dict in `startup/lib.py:21-23` both pass `conn=conn` to each stage — make sure stage_zero gets the *postgres* conn, not the target-DB conn, when the target DB is missing. Add a `stage_zero` override in `BACKEND_STAGES` (or a per-stage `dbname=` kwarg) to make the routing explicit.
  - Tests: add `py/tests/test_startup_creates_missing_database.py` — (a) target DB missing → startup creates it via `checkdatabase` and returns True; (b) target DB exists → startup skips creation and proceeds normally (regression); (c) role lacks CREATEDB → startup aborts with the two-line error from `checkcreatedb.py:78-85`, no `CREATE DATABASE` is issued; (d) reserved name → startup aborts, no `CREATE DATABASE`.
