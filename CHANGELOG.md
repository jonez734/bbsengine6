...
### fix(skin/tmpl): revert page-markdown.tmpl extends target from zoid6-page.tmpl to page.tmpl
...
The rename was, on inspection, **unnecessary**:

- `bbsengine6/skin/tmpl/page-markdown.tmpl` defines
  ...
  inheritance rule: a child can define a block the parent does
  not have; the parent renders normally around the child's
  content. There is no block-name collision.
- The parallel blurb-rendered-sections template,
  ...
  successfully on the .org vhost. That template also defines
  ...
  already extended bare `page.tmpl` successfully.
...
This commit reverts the rename so the handbook chapters inherit
the .org vhost's native `page.tmpl` chrome directly, with no
separate `zoid6-page.tmpl` rsync to keep in lock-step.

Changes:

- `bbsengine6/skin/tmpl/page-markdown.tmpl:1` —
  `{extends file="zoid6-page.tmpl"}` →
  `{extends file="page.tmpl"}`. The header comment block
  documenting the rename rationale is replaced with a
  `@since 2026-09-16` note explaining the revert.
- `bbsengine6/www/org/skin/tmpl/Makefile:39` — drop the
  ...
  trimmed; the `page-markdown.tmpl` + `youarehere.tmpl` +
...
Verification after the next `make wwworg` (which stages
`bbsengine6/www/org/skin/tmpl/page-markdown.tmpl` + the new
`page-markdown.tmpl` + `youarehere.tmpl` + `teos-breadcrumbs.tmpl`
to the .org docroot) and a `make skin-deploy-prod` (which stages
`bbsengine6/skin/tmpl/page-markdown.tmpl` to both docroots):

- `curl -sS https://www.bbsengine.org/handbook/6/QUICKSTART`
  returns 200 with the bbsengine.org chrome (pageheader,
  topbar, content, pagefooter) wrapping the rendered Markdown
  body.
- `bash bbsengine6/tests/test_handbook_6_returns_200.sh`
  continues to pass (the test's existing assertions are
  agnostic to the chrome; only the render-through-`page.tmpl`
  requirement is new and lives in the curl probe above).

Cross-ref: `engine/router.php::router_displayMarkdownFile()`
line 372 (`\bbsengine6\displaypage($data, 'page-markdown.tmpl',
false)`) is unchanged. The teos/com vhosts do not serve the
bbsengine6 handbook (`/handbook/<v>/...` rewrite rules are not
present in their htaccess-prod files — verified `grep -rn
handbook zoid6/sites/*/htaccess-prod` returns nothing), so the
change is invisible to them; if a future vhost starts serving
the handbook and ships a `page.tmpl` defining
`{block name="content"}` (as both the .org and the zoid6
templates do), the render will work correctly.
