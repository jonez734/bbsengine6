...
### fix(handbook): per-vhost top breadcrumb label on /handbook/

The "You are here" root crumb on the bbsengine.org/handbook/
vhost previously rendered as the literal string "teos" (and
the same string appeared twice in the breadcrumb trail) for
two related reasons:

1. `engine/router.php`'s `router_buildBreadcrumbs()` hardcoded
   the root crumb's title to `'teos'`. The blurb-driven
   `bbsengine6\blurb\buildbreadcrumbs()` had already migrated
   to read a per-vhost label via `TEOS_LABEL` (commit `ff78702`),
   but the filesystem-driven path used by the handbook render
   (`router_handleMarkdown` → `page-markdown.tmpl`) never did.
2. `skin/tmpl/teos-breadcrumbs.tmpl` (and its twin
   `breadcrumbs.tmpl`) called `{teos path=$b.path itemprop=true}`
   without passing `title=$b.title`. The `{teos}` Smarty plugin
   (`smarty/function.teos.php:25-44`) defaults its visible
   text to `end($uriSegments)` when no `title=` is supplied, so
   for the root crumb (path=`'teos'`, one segment) it rendered
   `'teos'` regardless of what `$b.title` carried.

In addition, when the handbook dispatch moved out of the
deleted `www/org/php/handbook.php` (commit `c40c79a`) into
`engine/router.php`, the matching `TEOS_LABEL="bbsengine6
handbook"` `putenv()` was lost; the new canonical entry point
never re-exported it.

Changes:

- `engine/router.php`: the `/handbook/<v>/` and
  `/handbook/<v>` dispatch blocks now `putenv('TEOS_LABEL=
  bbsengine6 handbook')` alongside the existing `TEOSDIR`/
  `TEOSURL` exports; `router_buildBreadcrumbs()` reads
  the label via `bbsengine6\util\vhost_label()` (new helper
  in `php/util.php`).
- `php/util.php`: new `bbsengine6\util\vhost_label()` helper
  (TEOS_LABEL env then constant, default `'teos'`); mirrors
  the resolution order of `bbsengine6\util\teos_url()` and
  `bbsengine6\util\handbook_home()`.
- `php/blurb.php`: `bbsengine6\blurb\getlabel()` now
  delegates to `bbsengine6\util\vhost_label()` (with a
  self-contained fallback that re-implements the env-then-
  constant logic for callers that load `blurb.php` without
  `util.php`).
- `skin/tmpl/teos-breadcrumbs.tmpl` and
  `skin/tmpl/breadcrumbs.tmpl`: both `{teos}` calls now
  pass `title=$b.title` so the breadcrumb data's `title`
  field (which `vhost_label()` populates) reaches the
  rendered link text.
- `php/test_breadcrumbs.php`: bootstrap order fixed (now
  requires `php/util.php` before `engine/router.php`, which
  is the same dependency order as the production entry
  point); the previously-broken breadcrumb-data tests
  (Tests 20-21) and the new TEOS_LABEL tests (Tests 22-24)
  all run.
- `tests/test_handbook_auth_bank_render.sh`: new `[7]`
  block parses the rendered breadcrumb `<ul>` and asserts
  the root crumb is `'bbsengine6 handbook'` linked to
  `/handbook/6/`, the last crumb links to the spec, and
  no `<a>` in the breadcrumb reads the literal `'teos'`
  (regression check for the pre-fix double-link bug).

The `/teos/` vhost is unaffected: with `TEOS_LABEL` unset,
`vhost_label()` falls through to the default `'teos'`, and
the template's `title=$b.title` matches `end($uriSegments)`
for all non-root crumbs (verified by `php/test_breadcrumbs.php`
Tests 10-15).
...
The previous (also-2026-09-07) commit
`fix(php): include engine/ in bootstrap include_path` tried to
fix the resulting `Failed opening required 'router.php'` fatal
by adding `dirname(__DIR__) . "/engine/"` to the bootstrap
include_path defaults. That entry pointed at
`/srv/www/bbsengine6/engine/` on the prod source tree — a
directory that the engine deploy never populates (it only ships
the tree to the docroots, not to the source tree). The
include_path entry therefore created a path that *looked*
correct on a config dump but resolved to an empty directory
at runtime. Confirmed on merlin: include_path included the
new entry, but `php-fpm` raised the same `Failed opening
required 'router.php'` fatal because no file matched.

The right shape is a relative require from the vhost docroot,
where `engine/Makefile deploy` actually lands the tree.

- `www/org/php/handbook.php` line 145: `require_once("router.php")`
  → `require_once __DIR__ . "/engine/router.php"`. The
  relative path resolves to
  `/srv/www/vhosts/www.bbsengine.org/html/engine/router.php`
  once `make engine-deploy-prod` has ssh-pushed the engine
  tree to the .org docroot. Makes the data dependency
  explicit and matches the engine/Makefile deploy target's
  canonical destination path.
- `php/bootstrap.php` `$defaults`: removed the
  `dirname(__DIR__) . "/engine/"` entry added in the previous
  commit. The entry pointed at a never-populated directory
  and gave a false sense of correctness on a config dump.
  Defaults are back to the four pre-this-incident entries
  (`__DIR__`, `dirname(__DIR__)`, `/srv/www/markdown/`,
  `/srv/www/smarty/`).
