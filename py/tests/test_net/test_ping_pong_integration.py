...
class TestPingPongIntegration:
    ...
    def test_ping_pong_exchange_5_seconds(self, free_port):
        ...
        def echo_server():
            ...
            try:
                ...
                while (
                    not stop_event.is_set() and time.time() - start_time < duration + 1
                ):
                    try:
                        ...
                        messages.append(
                            ("tx_pong", PACKET_TYPE_PONG, time.time() - start_time)
                        )
        ...
        def echo_client():
            ...
            try:
                ...
                while time.time() - start_time < duration:
                    ping_num += 1
                    ping = Packet(
                        ptype=PACKET_TYPE_PING, payload=str(ping_num).encode()
                    )
                    error = sender.send(ping)
                    messages.append(
                        ("tx_ping", PACKET_TYPE_PING, time.time() - start_time)
                    )
                    ...
                    try:
                        ...
                        if pong and not isinstance(pong, dict):
                            messages.append(
                                ("rx_pong", pong.ptype, time.time() - start_time)
                            )
                    except:
                        pass
        ...
        assert len(sent_pings) >= 20, (
            f"Expected ~25 pings in 5 seconds, got {len(sent_pings)}"
        )
    ...
    def test_ping_pong_exchange_role_reversal(self, free_port):
                ...
                while (
                    ...
                    try:
                        ...
                        if packet is None:
                            continue
                        messages.append(
                            ("srv_rx", packet.ptype, time.time() - start_time)
                        )
                        ...
                        messages.append(
                            ("srv_tx_ping", PACKET_TYPE_PING, time.time() - start_time)
                        )
                    except socket.timeout:
                        continue
        ...
        def client_initiator():
            ...
            try:
                ...
                while time.time() - start_time < duration:
                    ...
                    pong = Packet(
                        ptype=PACKET_TYPE_PONG, payload=str(pong_num).encode()
                    )
                    error = sender.send(pong)
                    messages.append(
                        ("cli_tx_pong", PACKET_TYPE_PONG, time.time() - start_time)
                    )
                    ...
                    try:
                        ...
                        if ping and not isinstance(ping, dict):
                            messages.append(
                                ("cli_rx_ping", ping.ptype, time.time() - start_time)
                            )
                    except:
                        pass
        ...
        assert len(cli_pongs) >= 20, (
            f"Expected ~25 pongs in 5 seconds, got {len(cli_pongs)}"
        )
