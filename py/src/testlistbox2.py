...
def selectresource(args, **kw):
    class EmpyreResourceListboxItem(object):
        ...
        def display(self):
            ...
            for elle in self.labels:
                io.echo(f"{{/all}}{{cha}} {{var:engine.menu.cursorcolor}}{{var:engine.menu.color}} {{var:engine.menu.boxcharcolor}}{{acs:vline}}{{var:cic}}", end="")
                io.echo(f" {elle.ljust(self.width-9, ' ')} {{/all}}{{var:engine.menu.boxcharcolor}}{{acs:vline}}{{var:engine.menu.shadowcolor}} {{var:engine.menu.color}} {{/all}}{{cha}}", end="", flush=True)
            return
    ...
    class EmpyreResourceListbox(listbox.Listbox):
        def __init__(self, args, **kw):
            ...
            for x in range(0, 30):
                ...
                rec["label"] = f"item {x}"
                rec["pk"] = x
                self.data.append(self.itemclass(rec, self.terminalwidth)) # f"item {x}")
            self.totalitems = len(self.data)

            super().__init__(args, title="select resource", data=self.data, pagesize=self.pagesize, itemclass=self.itemclass, totalitems=self.totalitems)
            io.echo(f"===> {self.totalitems=} {self.numpages=}", level="debug")

        def fetchpage(self):
            self.items = []
            for x in range(self.page*self.pagesize, self.pagesize+self.pagesize*self.page):
                print(f"{x=}")
    ...
    lb = EmpyreResourceListbox(args, title="select resource", itemclass=EmpyreResourceListboxItem) # player=kw["player"])
    res = lb.run("resource: ")
