...
def _validate_imap_server(server: Dict[str, Any]) -> None:
    ...
    if port < 1 or port > 65535:
        raise ConfigError(f"ImapServer '{name}': port must be 1-65535, got {port}")

    poll_interval = server.get("poll_interval", 30)
    if poll_interval < 1:
        raise ConfigError(f"ImapServer '{name}': poll_interval must be >= 1")

    timeout = server.get("timeout", 10)
    if timeout < 1:
        raise ConfigError(f"ImapServer '{name}': timeout must be >= 1")

    urgency = server.get("urgency", "ROUTINE")
    if urgency not in ("ROUTINE", "IMPORTANT", "URGENT", "CRITICAL"):
        raise ConfigError(f"ImapServer '{name}': invalid urgency {urgency}")

    recipients = server.get("recipients", [])
    if not recipients:
        raise ConfigError(f"ImapServer '{name}': recipients list cannot be empty")
...
def _validate_event_handler_config(event_type: str, handler: Dict[str, Any]) -> None:
    ...
    if urgency not in ("ROUTINE", "IMPORTANT", "URGENT", "CRITICAL"):
        raise ConfigError(f"EventHandler '{event_type}': invalid urgency {urgency}")
...
def load_config(config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        logging_config = _merge_config(data.get("logging", {}), _get_logging_defaults())
        database_config = _merge_config(
            data.get("database", {}), _get_database_defaults()
        )
        credentials_config = _merge_config(
            data.get("credentials", {}), _get_credentials_defaults()
        )
        ...
        for event_type, handler_data in events_data.get("handlers", {}).items():
            handler = _merge_config(handler_data, _get_event_handler_defaults())
