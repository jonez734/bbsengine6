...
def builddict(args, row):
  folder = {}
  for col in ("path", "uri", "title", "name", "intro", "attributes", "datecreated", "createdbymoniker", "dateapproved", "approvedbymoniker", "dateupdated", "updatedbymoniker"):
    if col in row:
      folder[col] = row[col]
  return folder
...
#    sig["approvedbymoniker"] = member.getcurrentmoniker(args)
  ...
  try:
    return database.insert(args, "engine.__folder", folder, returnid=True, primarykey="path", mogrify=mogrify, cur=cur)
...
#  cur = kwargs.get("cur", None)
#  mogrify = kwargs.get("mogrify", False)
  return database.update(args, "engine.__folder", path, folder, "path", **kwargs) # mogrify=mogify, cur=cur)
...
def input(prompt:str="folder: ", oldvalue:str="", **kw): # multiple:bool=True, verify:callable=allexist, **kw) -> str:
#  io.echo(f"bbsengine6.sig.input.120: {kw=}", level="debug")
  path = io.inputstring(prompt, oldvalue, **kw) # args=args, verify=verify, multiple=multiple, completer=Completer, returnseq=True, **kw)
  path = path.strip()
  return path
    ...
    with database.connect(args) as conn:
      with database.cursor(conn) as cur:
        for s in buildlist(args, buf):
          dat:tuple = (s,)
          io.echo(f"engine.folder.exists.100: {database.mogrifysql(cur, sql, dat)=}", level="debug")
          cur.execute(sql, dat)
          if cur.rowcount == 1:
            io.echo(f"engine.folder.exists.120: {buf=} exists", level="debug")
            return True
          io.echo(f"engine.folder.exists.140: {buf=} does not exist", level="debug")
          return False
  else:
    for s in buildlist(args, buf):
      dat:tuple = (s,)
      io.echo(f"engine.folder.exists.160: {database.mogrifysql(cur, sql, dat)=}", level="debug")
      cur.execute(sql, dat)
      if cur.rowcount == 1:
  ...
  if cur is None:
    with database.connect(args) as conn:
      with database.cursor(conn) as cur:
        ...
        dat = (buf,)
        cur.execute(sql, dat)
        io.echo(f"engine.folder.uriexists.120: {database.mogrifysql(cur, sql, dat)=}", level="debug")
        if cur.rowcount == 0:
          return False
        return True
  else:
