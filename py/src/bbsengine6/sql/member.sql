...
--  "id" bigserial unique not null primary key,
  "moniker" citext unique not null constraint chk_member_moniker_format check (moniker ~ '^[a-zA-Z0-9_]+(?::[a-zA-Z0-9_]+)?$'),
  "email" text not null,
  "password" text,
...
-- Idempotent migration: extend the chk_member_moniker_format constraint to
-- permit namespaced monikers of the form "<module>:<purpose>" (e.g.
-- "zoid6:casino", "empyre:router"). The CREATE TABLE above already uses the
-- extended pattern, but pre-existing databases that loaded this file before
-- the namespacing convention was introduced still carry the flat-form
-- constraint. The DO block below detects the old shape and replaces it; on
-- fresh DBs the constraint already matches and the block is a no-op.
do $$
begin
  if exists (
