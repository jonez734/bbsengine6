...
def help(args, **kwargs):
  ...
  if member["email"] is None:
    ...
    io.echo(f"{{red}} *{{var:optioncolor}}[E]{{var:labelcolor}} Email: {{var:valuecolor}}{member['email']} {{var:labelcolor}}(was: {{var:valuecolor}}{_member['email']})")
  else:
    io.echo(f"{{red}} *{{var:optioncolor}}[E]{{var:labelcolor}} Email: {member['email']}")
  ...
  if len(set_flags) > 0:
    io.echo(f"  {{var:optioncolor}}[F]{{var:labelcolor}} Flags:{{var:valuecolor}} {set_flags}")
  else:
...
def add(args, **kwargs) -> bool:
  ...
  with database.connect(args, pool=pool) as conn:
    ...
    member["createdbymoniker"] = libmember.getcurrentmoniker(args, conn=conn, **kwargs)
    io.echo(f"con.member.add.100: {member=}", level="debug")

    if args.debug is True:
      io.echo(f"con.member.add.120: {member=}", level="debug")

    moniker = libmember.insert(args, member, primarykey="moniker", conn=conn, **kwargs) # =member, mogrify=True, returnid=True, primarykey="moniker")
    if args.debug is True:
      io.echo(f"{moniker=}", level="debug")

    for name, data in member["flags"].items():
      io.echo(f"{name=} {data=}", level="debug")
      libmember.setflag(args, name, data["value"], moniker=moniker, conn=conn, **kwargs)
...
#    libmember.setattrs(args, {}, moniker, reset=True, conn=conn, **kwargs)
    if member["password"] is not None and member["password"] != "":
      libmember.setpassword(args, member["password"], moniker, conn=conn, **kwargs)
    libmember.setcredits(args, member["credits"], moniker, conn=conn, **kwargs)

    configurerole(args, member["loginid"], member["flags"]["SYSOP"]["value"], conn=conn, **kwargs)

    if io.inputboolean(f"{{var:promptcolor}}add member? {{var:optioncolor}}[Yn]: {{var:inputcolor}}", "Y") is False:
      conn.rollback()
      return False
...
#    io.echo(f"bbsengine.con.member.100: {conn=}", level="debug")
    ...
    while not done:
        ...
        ch = io.inputchoice(f"{{var:promptcolor}}member:{{var:inputcolor}} ", "NAEQX", "Q")
        if ch == "E":
          io.echo("Edit")
