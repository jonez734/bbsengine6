    ...
    try:
      with database.connect(args, pool=pool, **kwargs) as conn:
        ...
        if lib.checksuperuser(args, conn=conn, **kwargs) is False:
          io.echo(f"{{var:valuecolor}}no permission to create the database", level="error")
          return False

        if lib.checkwebserverrole(args, conn=conn, **kwargs) is False:
          io.echo(f"{{var:labelcolor}}check of {{var:valuecolor}}www-data{{var:labelcolor}} failed", level="error")
          return False
...
def main(args, **kwargs):
  ...
  with database.getpool(args) as pool:
    ...
    while not done:
      ...
      if membercount > 0:
        session.updatelastactivity(args, session.currentsessionid, conn=conn, **kwargs)
      else:
      ...
      ch = io.inputchoice("{var:promptcolor}console: {var:inputcolor}", "SMXQ", "X")
      if ch == "M":
        io.echo("Members")
