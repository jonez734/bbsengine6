"""
bbsengine6/games/base.py
Game-type catalog and abstract ``BaseGame`` for bbsengine6 clients.

Moved from ``casino.games.base`` so the menu-derivation helper in
``bbsengine6.games.menu`` can depend on a bbsengine6-owned catalog
...
it is a generic engine primitive; concrete games live in their own
packages (e.g. ``casino.blackjack``, ``casino.poker``).
"""
