...
class Frame:
    ...
    def __init__(self, data: bytes, width: int, height: int, frame_id: int = 0):
        ...
        Args:
            data: Frame data as bytes (width * height * 3 for RGB)
    ...
    def from_size(width: int, height: int, fill: int = 0, frame_id: int = 0) -> "Frame":
        """Create a frame filled with a value."""
...
class NumpyFrame:
    ...
    def __init__(self, data: Any, frame_id: int = 0):
        ...
        Args:
            data: numpy array of shape (height, width, 3) and dtype uint8
            frame_id: Optional frame identifier
        """
        if not HAS_NUMPY:
...
def frame_from_any(
    ...
    Args:
        ...
        width: width (required for bytes, ignored for numpy)
        height: height (required for bytes, ignored for numpy)
        frame_id: frame identifier
    
    Returns:
        NumpyFrame if data is numpy array, Frame if data is bytes
    """
