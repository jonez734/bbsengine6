...
def getcurrentmoniker(args, **kwargs):
  ...
  loginid = util.getcurrentloginid(args, **kwargs) # works on windows, too. @project:8158
...
def getcurrentid(args, **kwargs):
  ...
  try:
    ...
    io.echo(f"bbsengine6.member.getcurrentid.180: database error: {e}", level="error")
    raise
...
def setcredits(args, amount:int, moniker:str=None, **kwargs):
    ...
    if moniker is None:
        ...
        if moniker is None:
            io.echo("bbsengine.member.setcredits.120: You do not exist! Go Away!", level="error")
            return None
...
def getcredits(args, membermoniker:str=None, **kwargs) -> int:
  ...
  if membermoniker is None:
    ...
    if membermoniker is None:
      io.echo("bbsengine.member.getcredits.100: You do not exist! Go Away!", level="error")
      return None
  ...
  try:
    ...
    if conn is None:
      pool = kwargs.get("pool", None)
      if pool is None:
        io.echo(f"bbsengine6.member.getcredits.200: pool is None", level="error")
        return None
      with database.connect(args, pool=pool) as conn:
        return _work(conn)
    return _work(conn)
...
def update(args, member, moniker=None, **kwargs):
  ...
  def _work(conn):
    database.update(args, "engine.__member", moniker, rec, primarykey="moniker", mogrify=True, conn=conn)
...
##    io.echo(f"getcurrentmoniker done {moniker=}", level="debug")
    if moniker is None:
      io.echo("bbsengine.member.checkflag.100: You do not exist! Go away!", level="error")
      return None
  ...
  def _work(conn):
    ...
    try:
      with database.cursor(conn) as cur:
        ...
        flags = {flag['name']: {'description': flag['description'], 'value': flag['value']} for flag in cur.fetchall()}
  ...
  try:
    ...
    if conn is None:
      ...
      if pool is None:
        ...
        if conn is None:
          io.echo(f"bbsengine.member.getflags.220: unable to connect()", level="error")
          return None
        with conn:
          return _work(conn)
    return _work(conn)
...
#  conn = kwargs.get("conn", None)
  ...
  return database.insert(args, table, cols, **kwargs) # table, member, returnid=returnid, primarykey=primarykey, mogrify=mogrify)
