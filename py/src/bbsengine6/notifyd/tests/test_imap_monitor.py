    ...
    def test_poll_new_emails(self):
        ...
        with patch("bbsengine6.notifyd.imap_monitor.connect_imap") as mock_connect:
            ...
            mock_imap.uid.side_effect = [
                ...
                ("OK", [(b"4 (RFC822 {20})", b"From: a@ex.com\nSubject: Email 4\n\nBody 4")]),  # Fetch 4
                ("OK", [(b"5 (RFC822 {20})", b"From: b@ex.com\nSubject: Email 5\n\nBody 5")]),  # Fetch 5
            ]
...
class TestPollImapAllMailboxes:
    ...
    def test_poll_all_mailboxes_retrieves_last_uid(self):
        ...
        mock_get_last_uid = MagicMock(side_effect=lambda mb, srv: int(mb[-1]) if mb[-1].isdigit() else 0)
