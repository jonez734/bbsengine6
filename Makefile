...
# Canonical install for php/ skin/ smarty/ js/ (engine entry points ship via ENGINESTAGEDOCROOT).
export ENGINESTAGE = /srv/www/bbsengine6/
export ENGINEPROD = $(ENGINEHOST):$(ENGINESTAGE)
...
wwworg:
	...
	$(MAKE) engine-deploy-prod \
		...
		ENGINEPRODDOCROOT=$(ORGHOST):/srv/www/vhosts/www.bbsengine.org/html/engine/
