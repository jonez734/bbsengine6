...
class TestModuleWithCustomArgs(unittest.TestCase):
    ...
    def test_module_buildargs_returns_parser(self):
        ...
        parser.add_argument(
            "--filter", choices=["all", "active", "sysop"], help="Filter results"
        )
        parser.add_argument(
...
class TestCustomArgsIntegration(unittest.TestCase):
    ...
    def test_full_flow_with_custom_args(self, mock_check, mock_runcallback, mock_load):
        ...
        parser = argparse.ArgumentParser(description="Test module")
        parser.add_argument(
            "--filter", choices=["all", "active", "sysop"], default="all"
        )
        ...
        # Run with custom arg
        result = module.run(self.mock_args, "test.module", argv=["--filter", "sysop"])
    ...
    @patch("bbsengine6.module.runcallback")
    @patch("bbsengine6.module.check")
    def test_invalid_custom_arg_returns_false(
        self, mock_check, mock_runcallback, mock_load
    ):
        ...
        result = module.run(
            self.mock_args, "test.module", argv=["--filter", "invalid_choice"]
        )
